...
These are instructions every developer (including AI agents) must follow when creating, maintaining, and testing our app. Follow them strictly to ensure quality, readability, and maintainability.

## Coder’s Notes
Every file must start with a comment block matching the exact format:
```javascript
//
// File: <filename>
// Author: <author_name>
...
## Project Structure
All project files should be organized clearly. Keep your source code inside a `src` folder, with separate subfolders for:
- `models` (data models)
...
- Place all unit and integration tests in a `tests` folder.
- Place documentation in a `docs` folder.
- Place configuration files in a `config` folder.
- Place images, fonts, or other media in an `assets` folder.
